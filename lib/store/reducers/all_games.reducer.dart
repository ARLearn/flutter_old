...
AllGamesState _reduceParticipateGamesResponse(AllGamesState gamesState,
    LoadParticipateGamesListResponseAction action) {
...
AllGamesState _reduceGameResponse(AllGamesState gamesState,
    LoadParticipateGameResponseAction action) {
  //if (!action.isError()) {
    //Game toAdd = ();
    gamesState.idToGame[action.game.gameId] = action.game;
    return gamesState.copyWith(
        i2g: HashMap<int, Game>.from(gamesState.idToGame)
    );
//  }
  return gamesState;
}
