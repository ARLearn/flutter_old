...
final runIdSelector = (RunState state) => state != null && state.run != null ? state.run.runId : -1;
final currentRunSelector = (RunState state) => state?.run;
final currentRunSel =
    (AppState state) => runStateFeature(state) != null ? runStateFeature(state).run : null;
...
final Selector<AppState,  HashMap<String, ARLearnAction>> localAndUnsyncActions =
    createSelector2(actionsFromServerSel, currentRunActionsSelector, (server, List<ARLearnAction> local) {
...
final Selector<AppState, bool> correctAnswerGivenSelector = createSelector2(localAndUnsyncActions, currentItemId, (HashMap<String, ARLearnAction> actionsFromServer, int id){
...
final Selector<AppState, List<Response>> currentItemResponsesFromServerAsList = createSelector3(
    runStateFeature,
    responsesFromServerFeature, currentItemId, (dynamic runState, HashMap<int, Response> map, int itemId) {
  return map.values.where((response) => response.generalItemId == itemId).toList(growable: true);
});
