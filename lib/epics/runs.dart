...
Stream<dynamic> _gameParticipateStream(
  ...
  return actions.where((action) => action is ApiRunsParticipateAction).asyncMap(
      (action) =>
          RunsApi.participate(action.gameId)
              .then((results) =>
                  new ApiResultRunsParticipateAction(runs: results, gameId: action.gameId)));
//              .catchError((error) =>
//                  new ApiResultRunsParticipateAction(error, action.gameId)));
}
